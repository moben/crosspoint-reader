...
### 1. Orchestrator: `renderCharImpl<Orientation, Mode>`

The main entry point for glyph rendering. It handles setup, clipping, and the outer loop.

- **Hoisting**: 
  - Calculate the **anchor** (physical starting $x, y$ coordinates) once.
  - Calculate **effective width/height** by clipping the glyph dimensions to the active strip bounds (hoisted outside the loops).
- **Orientation-Aware Outer Loop (`constexpr`)**:
  - **Portrait Mode**: Iterate over **glyph width** ($X$). Each iteration represents a logical column.
  - **Other Modes**: Iterate over **glyph height** ($Y$). Each iteration represents a logical row.
- **Dispatch**: Call `renderCharRow<Orientation, Mode>(...)` for each iteration of the outer loop.

### 2. Orientation-Aware Loop Dispatch (Main `renderCharImpl` body)

Both `orientation` and `renderMode` are template parameters, so the compiler can eliminate
all dead branches — no runtime `switch` in the pixel loop.
...
static void renderCharImpl(const GfxRenderer& renderer, ...) {
    ...
    // --- Hoisted: anchor & clipping ---
    // Calculate effective W/H clipped to strip bounds
    int effW = clipToStripBounds(glyphWidth, ...);
    int effH = clipToStripBounds(glyphHeight, ...);
    
    // Calculate physical anchor (starting coordinates) once
    int anchorX, anchorY = computeAnchor<orientation>(...);

    // --- Orientation-Aware Outer Loop ---
    if constexpr (orientation == GfxRenderer::Portrait) {
        // Portrait: iterate over glyph width (logical columns)
        for (int gx = 0; gx < effW; gx++) {
            renderCharRow<orientation, renderMode>(..., gx, effH);
        }
    } else {
        // Other modes: iterate over glyph height (logical rows)
        for (int gy = 0; gy < effH; gy++) {
            renderCharRow<orientation, renderMode>(..., gy, effW);
        }
    }
}
```

### 3. Specialized Row Handlers: `renderCharRow<Orientation, Mode>`

These templates handle the mapping from logical glyph coordinates to physical framebuffer indices.

- **Orientation-Aware Inner Loop (`constexpr-if`)**:
  - **Portrait Mode**: Iterate over **glyph height** ($Y$). Each iteration writes a whole framebuffer byte to a **logical column**.
  - **Other Modes**: Iterate over **glyph width** ($X$). Each iteration writes a whole framebuffer byte to a **logical row**.
- **Stride & Direction Logic**:
  - Uses `glyphWidth` and `glyphHeight` as strides to jump through the bitmap.
  - Uses `constexpr-if` to select the correct index-increment direction based on orientation.
- **Memory Efficiency**:
  - No heap allocation.
  - `constexpr-if` ensures the compiler eliminates non-active orientation paths, preventing code bloat.

### 4. Coordinate & Stride Logic

To support efficient byte-based writing, the row helpers will use:
- **Glyph Stride**: Used to jump through the bitmap data based on the current orientation.
- **Orientation Direction**: Determines whether we increment/decrement indices to find the next logical pixel.
- **Byte Alignment**: Ensures that each inner loop iteration processes exactly one framebuffer byte (8 pixels), applying head/tail masks where necessary.

### 5. Grayscale Pass Architecture
...
### 8. Compile-Time Orientation Specialization
